  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (data.isProfane) {
         ...
         if (newStrikes >= 3) {
            setIsDisabled(true);
            setHistory(prev => [...prev, { role: 'model', text: "Chat disabled due to multiple violations of our professional conduct policy." }]);
            setIsLoading(false);
            return;
         }
      }
      ...
      let errorMessage = "I'm sorry, I'm having a bit of trouble coming up with a response right now. Could you try rephrasing that? 😅 You can also reach out to me directly at lacuarindaniela1@gmail.com!";
      
      const errorStr = error?.message?.toLowerCase() || "";
      if (errorStr.includes("not configured") || errorStr.includes("api key") || errorStr.includes("expired")) {
        errorMessage = "The AI service is unavailable because the API key is expired or invalid. Please check your Gemini API key settings.";
      } else if (errorStr.includes("temporarily unavailable:")) {
        // Extract the specific error after the colon
        const parts = error.message.split("temporarily unavailable:");
        if (parts.length > 1) {
          errorMessage = `Service Issue: ${parts[1].trim()}`;
        }
      ...
      <AnimatePresence>
        {isOpen && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center md:items-end md:justify-end p-4 md:p-8">
            ...
            >
              {/* Header */}
              <div className="p-4 border-bottom border-gray-100 flex items-center justify-between bg-white">
              ...
              >
                {history.length === 0 && (
                  <div className="flex flex-col items-center justify-center h-full text-center p-8">
                     ...
                     <h4 className="font-display text-sm font-bold text-black mb-2">How can I help you today?</h4>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="bg-white p-3 rounded-2xl rounded-bl-none border border-gray-100 shadow-sm">
                      <div className="flex gap-1">
                        <motion.span animate={{ opacity: [0.2, 1, 0.2] }} transition={{ repeat: Infinity, duration: 1 }} className="w-1.5 h-1.5 bg-gray-400 rounded-full" />
                        <motion.span animate={{ opacity: [0.2, 1, 0.2] }} transition={{ repeat: Infinity, duration: 1, delay: 0.2 }} className="w-1.5 h-1.5 bg-gray-400 rounded-full" />
                        <motion.span animate={{ opacity: [0.2, 1, 0.2] }} transition={{ repeat: Infinity, duration: 1, delay: 0.4 }} className="w-1.5 h-1.5 bg-gray-400 rounded-full" />
                      </div>
                    </div>
                  </div>
                )}
